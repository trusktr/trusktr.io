...
exports.default = getWebGlRenderer;

var _webglUtils = require('./webglUtils');

const updateResolution = function (state) {
    const resolution = [parseFloat(getComputedStyle(state.gl.canvas).width) * window.devicePixelRatio, parseFloat(getComputedStyle(state.gl.canvas).height) * window.devicePixelRatio, 1000];

    _webglUtils.setGlResolution.apply(void 0, [state.gl].concat(resolution));
    state.projectionMatrix = _webglUtils.m4.perspective(45, resolution[0] / resolution[1], 1, 2000);
};

var WebGlRenderer = function WebGlRenderer() {};

WebGlRenderer.prototype.initGl = function initGl(scene) {
    const gl = (0, _webglUtils.createWebGLContext)(scene);
    ...
    const vertShader = (0, _webglUtils.createShader)(gl, gl.VERTEX_SHADER, _webglUtils.vertShaderSource);
    const fragShader = (0, _webglUtils.createShader)(gl, gl.FRAGMENT_SHADER, _webglUtils.fragShaderSource);
    const program = (0, _webglUtils.createProgram)(gl, vertShader, fragShader);
    ...
    state.projectionMatrix = _webglUtils.m4.identity;

    updateResolution(state);
    scene.on('parentsizechange', function () {
        return updateResolution(state);
    });
...
WebGlRenderer.prototype.drawScene = function drawScene(scene) {
    ...
    state.lightWorldPosition = [300 * Math.sin(state.lightAnimParam), 300 * Math.sin(state.lightAnimParam * 2), Math.abs(300 * Math.cos(state.lightAnimParam))
    //300
    ];
    ...
    if (typeof backgroundColor == 'string') {
        backgroundColor = backgroundColor.split(' ').map(function (rgbPart) {
    ...
    //state.cameraAngle++
    let cameraMatrix = _webglUtils.m4.identity;
    cameraMatrix = _webglUtils.m4.multiply(cameraMatrix, _webglUtils.m4.yRotation(state.cameraAngle));
    cameraMatrix = _webglUtils.m4.multiply(cameraMatrix, _webglUtils.m4.translation(0, 0, state.cameraRadius * 1.5));
    const viewMatrix = _webglUtils.m4.inverse(cameraMatrix);

    state.viewProjectionMatrix = _webglUtils.m4.multiply(state.projectionMatrix, viewMatrix);
...
WebGlRenderer.prototype.drawNodeAndRecurse = function drawNodeAndRecurse(state, node) {
    ...
    if (meshAttr) {
        ...
        const svgElement = Array.from(node.element.children).find(function (child) {
        ...
        if (meshAttr == 'cube') {
            if (!(node.__shape instanceof _webglUtils.Cube)) {
                node.__shape = new _webglUtils.Cube(0, 0, size.x);
            }
            // TODO else, like quad or symtrap
        } else if (meshAttr == 'quad') {
            if (!(node.__shape instanceof _webglUtils.Quad)) {
                node.__shape = new _webglUtils.Quad(size.x, size.y);
            } else {
            ...
            if (hasTexture) {
                ...
                if (!node.__texture) {
                    ...
                    node.__shape.textureCoordinates = new Float32Array([0, 0, 1, 0, 1, 1, 1, 1, 0, 1, 0, 0]);
            ...
            if (!(node.__shape instanceof _webglUtils.IsoscelesTriangle)) {
                node.__shape = new _webglUtils.IsoscelesTriangle(size.x, size.y);
            }
            // TODO else, like quad or symtrap
        } else if (meshAttr == 'pyramid4') {
            if (!(node.__shape instanceof _webglUtils.FourSidedPyramid)) {
                node.__shape = new _webglUtils.FourSidedPyramid(size.x, size.y);
            }
            // TODO else, like quad or symtrap
        } else if (meshAttr == 'symtrap') {
            if (!(node.__shape instanceof _webglUtils.SymmetricTrapezoid)) {
                node.__shape = new _webglUtils.SymmetricTrapezoid(size.x / 2, size.x, size.y);
            } else {
        ...
        if (node.__shape) {
            ...
            gl.vertexAttribPointer(state.colorAttributeLocation, colorSize, colorType, normalizeColorData, colorStride, colorOffset);
            ...
            gl.vertexAttribPointer(state.vertexAttributeLocation, vertexSize, type, normalizeVertexData, stride, offset);
            ...
            gl.vertexAttribPointer(state.normalAttributeLocation, normalSize, normalType, normalizeNormalsData, normalStride, normalOffset);
            ...
            if (hasTexture) {
                ...
                gl.vertexAttribPointer(state.textureCoordinateLocation, textureCoordinateSize, textureCoordinateType, normalizeTextureCoordinateData, textureCoordinateStride, textureCoordinateOffset);
            ...
            const worldViewProjectionMatrix = _webglUtils.m4.multiply(state.viewProjectionMatrix, node._worldMatrix.toFloat32Array());
...
function getWebGlRenderer() {
    if (instance) {
        ...
        return instance = new WebGlRenderer();
    }
}
